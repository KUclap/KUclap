...
import ShareModal from "../async/ShareModal";
...
const ButtonIcon = styled.button`
	...
	&:hover {
		svg {
			#clap {
				fill: ${(props) => (props.valueAction === false ? blue : light_blue)};
			}
			#boo {
				fill: ${(props) => (props.valueAction === false ? red : sea_pink)};
			}
			#arrow {
				fill: ${light_blue};
				stroke: ${light_blue};
		...
		<Actions>
			<ButtonContainer>
				...
				{clapCounter === prevClapCounter ? (
					...
					<NumberAction color={blue}>{`+${clapCounter - prevClapCounter}`}</NumberAction>
				)}
			</ButtonContainer>
			<ButtonContainer>
				<ButtonIcon
				...
				{booCounter === prevBooCounter ? (
					...
					<NumberAction color={red}>{`+${booCounter - prevBooCounter}`}</NumberAction>
				)}
			</ButtonContainer>
			<ButtonWithIcon onClick={() => setShareModal(true)} type="share">
				<BodyTiny>แชร์</BodyTiny>
				<Share />
			</ButtonWithIcon>
			<ShareModal
